((window, factory) => {
  ...
  class ChromeTabs {
    ...
    get tabContentWidths() {
      ...
      const flooredClampedTargetWidth = Math.floor(clampedTargetWidth)+30
      const totalTabsWidthUsingTarget = ((flooredClampedTargetWidth+30) * numberOfTabs)// + (2 * TAB_CONTENT_MARGIN) - tabsCumulativeOverlappedWidth
    ...
    layoutTabs(createLeft, drag) {
      ...
      this.tabPositions.forEach((position, i) => {
        if (i+1==this.tabPositions.length) {
          styleHTML+= `
            .chrome-tabs-create {
              left: ${createLeft||(((position-30*(i))+((tabContentWidths[0] + (2 * TAB_CONTENT_MARGIN))-40))+7)}px !important;
        ...
        styleHTML += `
          .chrome-tabs[data-chrome-tabs-instance-id="${ this.instanceId }"] .chrome-tab:nth-child(${ i + 1 }) {
            transform: translate3d(${ position-30*(i) }px, 0, 0);
          }
        `
      })
    ...
    addTab(tabProperties, { animate = true, background = false } = {}) {
      ...
      this.tabContentEl.insertBefore(tabEl, document.querySelector('.chrome-tabs-create'))
      this.setTabCloseEventListener(tabEl)
      this.updateTab(tabEl, tabProperties)
      this.emit('tabAdd', { tabEl, url:tabProperties.url, oUrl: tabProperties.fakeUrl })
    ...
    removeTab(tabEl) {
      if (tabEl === this.activeTabEl) {
        if (tabEl.nextElementSibling&&tabEl.nextElementSibling.classList.contains('chrome-tab')) {
    ...
    updateTab(tabEl, tabProperties) {
      ...
      if (tabProperties.favicon) {
        ...
        faviconEl.style.backgroundImage = `url('/ico/dark.ico')`
        faviconEl.removeAttribute('hidden', '')
      }
      ...
      if (tabProperties.opener) {
        tabEl.setAttribute('data-opener', tabProperties.opener)
      }
    }
    ...
    setupDraggabilly() {
      ...
      tabEls.forEach((tabEl, originalIndex) => {
        ...
        draggabilly.on('dragMove', _ => {
          var tr = tabEl.style.transform;
          var rawDistance = parseInt(tr.replace(/translate3d\((.*?)px,\s*.*?px,.*?\s*\)/, '$1'))
          var left = parseInt(getComputedStyle(document.querySelectorAll('.chrome-tabs-create')[0]).left.replace('px',''));
            var distance = (rawDistance+parseInt(tabEl.style.width.replace('px',''))+parseInt(tabEl.style.left.replace('px','')))

          if (distance>left) this.layoutTabs(distance+10, true)
          else this.layoutTabs(distance+10, true)
        })
    ...
    animateTabMove(tabEl, originIndex, destinationIndex) {
      if (destinationIndex < originIndex) {
        ...
        tabEl.parentNode.insertBefore(tabEl, this.tabEls[destinationIndex+1]||document.querySelector('.chrome-tabs-create'))
      }
      this.emit('tabReorder', { tabEl, originIndex, destinationIndex })
      //this.layoutTabs(null, true)
    }
  }
